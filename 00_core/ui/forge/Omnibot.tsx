...
// GRAFT: Dependency severed for stability
// import { StreamEvent, RunStartEvent, AgentThinkEvent, RunFinalEvent } from '../../lib/stream_events'; 

export type RunFinalEvent = {
    type: "run.final";
    payload: {
        final_output: string;
        summary?: string;
...
// Use 'any' for others temporarily to unblock render
type StreamEvent = any;
type RunStartEvent = any;
type AgentThinkEvent = any;
...
export const Omnibot: React.FC<SidebarCoPilotProps> = ({ currentContent, onUpdateContent, onClose }) => {
    const [messages, setMessages] = useState<Message[]>([
        { id: '1', role: 'assistant', text: 'Omnibot Online. Mode: EDITOR. How can I help?', timestamp: Date.now() }
    ]);
    ...
    useEffect(() => {
        const handleCLIEvent = (event: any) => {
            // GRAFT: Listen for CLI Receipts
            if (event.type === "run.final") {
                console.log("[OMNIBOT] Receipt:", event.output);
                // Force UI update
                setReceipts(prev => [...prev, event.output]);
            }
        };

        window.addEventListener('message', (e) => {
            if (e.data && e.data.type) {
                handleCLIEvent(e.data);
            }
        });

        return () => {
            window.removeEventListener('message', handleCLIEvent);
        };
    }, []);
